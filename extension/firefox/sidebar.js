...
async function loadRuntimeContext() {
  const response = await browser.runtime.sendMessage({ type: "AI_ASSIST_GET_RUNTIME_CONTEXT" });
...
async function runGoogleConnectAction(action = "connect") {
  ...
  GOOGLE_CONNECT_BUTTON.disabled = true;
  const response = await browser.runtime.sendMessage({ type: action === "reconnect" ? "AI_ASSIST_GOOGLE_RECONNECT" : "AI_ASSIST_GOOGLE_CONNECT" });
